...
def setup(args):
    ...
    state_dict = torch.load(os.path.join(args.dataset_dir,'model_checkpoint.pth'))
...
def valid(args, model_grd, model_sat, writer, test_loader, global_step):
    ...
    sat_global_descriptor = torch.zeros([1249, 768]).to(args.device)
    grd_global_descriptor = torch.zeros([1249, 768]).to(args.device)
    val_i =0
    with torch.no_grad():
